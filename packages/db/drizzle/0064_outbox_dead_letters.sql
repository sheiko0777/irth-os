...
-- stuck in place. No RLS, matching outbox_events itself -- this table is
-- read only by the worker's own trusted role, never through a tenant-scoped
-- session (see processOutbox's own comment on why outbox_events has none).
--
