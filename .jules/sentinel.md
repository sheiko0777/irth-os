...
## 2026-08-06 - Prevent Unhandled Exceptions in JSON.parse
**Vulnerability:** Webhook endpoints parsing raw request bodies with `JSON.parse()` without a `try...catch` block would throw unhandled exceptions for invalid JSON payloads.
**Learning:** This exposes the application to DoS and unhandled crashes. Defensive code must be used to catch and handle malformed input gracefully.
**Prevention:** Always wrap `JSON.parse()` in a `try...catch` block when parsing untrusted user or webhook input, returning a 400 Bad Request instead of allowing a 500 error.
