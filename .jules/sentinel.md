...
## 2024-07-02 - Unprotected JSON.parse in Webhooks
**Vulnerability:** Raw webhook payloads were parsed using `JSON.parse()` without a `try...catch` block in `apps/api/src/routes/webhooks/paymob.ts` and `apps/api/src/routes/webhooks/bosta.ts`.
**Learning:** If a webhook receives malformed JSON, an unhandled exception will be thrown, resulting in a 500 Internal Server Error. This can lead to service instability and is a Denial of Service (DoS) risk if an attacker repeatedly sends invalid payloads.
**Prevention:** Always wrap `JSON.parse()` calls on incoming payload strings in a `try...catch` block to handle exceptions gracefully and return a 400 Bad Request error.
