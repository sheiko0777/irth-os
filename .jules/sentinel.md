...
## 2025-02-24 - Missing Authorization Guard on Products API Endpoints
**Vulnerability:** The API endpoints for creating, updating, and deleting products in `apps/api/src/routes/products.ts` used `requireRole` instead of the centralized `requirePermission` middleware. This bypassed the authorization matrix defined in `packages/db/src/permissions.ts`. Additionally, the view endpoints were missing `requireOrgId()`, which could lead to issues.
**Learning:** Using `requireRole` directly instead of checking against the established `requirePermission(resource, action)` skips the centralized role definition mechanism for resources. Furthermore, `requirePermission` must always be composed sequentially *after* `requireOrgId()` to establish necessary tenant isolation and set context parameters.
**Prevention:** Always secure API endpoints mapping to resources defined in the authorization matrix with a combination of `requireOrgId()` and `requirePermission(resource, action)`.
