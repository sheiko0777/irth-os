...
## 2025-02-24 - Authorization Bypass in API Notifications Endpoints
**Vulnerability:** The API endpoints for reading and managing notifications (`GET /`, `PATCH /:id/read`, `PATCH /read-all`) in `apps/api/src/routes/notifications.ts` only used the `requireOrgId()` middleware, meaning any authenticated user could access them regardless of their role.
**Learning:** While `requireOrgId()` enforces tenant isolation, it does not verify if the authenticated session actually holds a valid role within the organization. Even for operations that should be accessible to all members (like reading their own notifications), explicit role-based access control (RBAC) middleware is required to prevent an authenticated but unassigned user from proceeding.
**Prevention:** Always apply the `requireRole(owner, admin, member)` middleware to endpoints that are intended to be accessible to all valid members of an organization, but are not mapped to a specific resource in the centralized authorization matrix (`packages/db/src/permissions.ts`).
