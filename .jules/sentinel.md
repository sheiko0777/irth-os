...
## 2024-06-25 - Missing Authorization Guard on Shipping Create API Endpoint
**Vulnerability:** The API endpoint for creating shipping fulfillments in `apps/api/src/routes/shipping.ts` (`POST /create`) only used the `requireOrgId()` middleware, meaning any authenticated member of the organization could access and invoke it regardless of their specific permissions.
**Learning:** `requireOrgId()` only enforces tenant isolation, preventing cross-organization access and extracting context. It does not enforce role-based access control (RBAC) within the organization itself. If an endpoint requires a specific permission per the matrix in `packages/db/src/permissions.ts`, it must use `requirePermission()`. However, `requirePermission()` depends on context set by `requireOrgId()`, so they must be composed sequentially (e.g. `requireOrgId(), requirePermission('orders', 'write')`).
**Prevention:** Always use `requirePermission(resource, action)` *after* `requireOrgId()` for endpoints that expose or manipulate resources mapped in the authorization matrix.
