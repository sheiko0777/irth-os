...
export function verifyHmac(secretEnvKey, headerName) {
    return async (c, next) => {
        const secret = process.env[secretEnvKey];
        if (!secret)
            return c.json({ data: null, error: 'Webhook secret not configured', meta: null }, 500);
        const signature = c.req.header(headerName);
        if (!signature)
            return c.json({ data: null, error: 'Missing signature', meta: null }, 401);
