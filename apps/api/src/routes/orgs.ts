...
orgsRouter.get("/:id/members", async (c: Context) => {
  try {
    ...
    const members = await db
      .select()
      .from(orgMembers)
      .where(eq(orgMembers.orgId, id as string));
...
orgsRouter.post(
  ...
  async (c: Context) => {
    try {
      ...
      if (!org) {
        return c.json(
          { data: null, error: "Organization not found", meta: null },
          404,
        );
      }
      ...
      console.log(
        `Sending email to ${email} with subject: دعوة للانضمام إلى ${org.name}`,
      );
...
orgsRouter.post("/invite/accept", async (c: Context) => {
  try {
    ...
    const [invite] = await db
      ...
      .where(
        and(
          ...
          eq(orgInvites.orgId, c.req.header("org_id") || "system"),
        ),
      );
    if (!invite) {
    ...
    await db
      .delete(orgInvites)
      .where(
        and(eq(orgInvites.id, invite.id), eq(orgInvites.orgId, invite.orgId)),
      );
