...
const getOrgId = (c) => c.get('orgId') || '00000000-0000-0000-0000-000000000000';
const getUserId = (c) => c.get('userId') || '00000000-0000-0000-0000-000000000000';
const createShippingSchema = z.object({
    orderId: z.string().uuid()
});
shippingRoute.post('/create', async (c) => {
    ...
    try {
        const bostaRequest = await fetch('https://app.bosta.co/api/v0/deliveries', {
            ...
            body: JSON.stringify({
                ...
                // Normally these would come from customer profile/settings mapped from the order
                returnAddress: { firstLine: "Store Address" },
                dropOffAddress: { firstLine: "Customer Address" },
                receiver: { firstName: "Customer", phone: "01000000000" },
                webhookUrl: `${process.env.PUBLIC_API_URL || 'https://api.irth.com'}/api/webhooks/bosta?orgId=${orgId}` // Pass orgId in URL since Bosta might not send custom fields
            })
        });
        if (!bostaRequest.ok) {
            throw new Error(`Bosta API error: ${bostaRequest.status}`);
        }
        const bostaResponse = await bostaRequest.json();
