...
notificationsRouter.get('/', async (c) => {
    ...
    if (!orgId) {
        return c.json({ data: null, error: 'Missing org_id header', meta: null }, 401);
    }
    try {
        const data = await db
            .select()
        ...
        return c.json({ data: null, error: error instanceof Error ? error.message : String(error), meta: null }, 500);
    }
});
...
notificationsRouter.patch('/:id/read', async (c) => {
    ...
    try {
        const result = await db
            ...
            .set({ read: true })
            .where(and(eq(notifications.id, notificationId), eq(notifications.orgId, orgId), eq(notifications.userId, userId)))
            .returning();
        if (result.length === 0) {
