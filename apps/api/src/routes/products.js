...
productsRouter.post('/', requireRole('owner', 'admin'), async (c) => {
    try {
        const orgId = c.req.header('org_id');
        if (!orgId)
            return c.json({ data: null, error: 'Unauthorized', meta: null }, 401);
...
productsRouter.patch('/:id', requireRole('owner', 'admin'), async (c) => {
    try {
        const orgId = c.req.header('org_id');
        if (!orgId)
            return c.json({ data: null, error: 'Unauthorized', meta: null }, 401);
...
productsRouter.post('/:id/variants', requireRole('owner', 'admin'), async (c) => {
    try {
        const orgId = c.req.header('org_id');
        if (!orgId)
            return c.json({ data: null, error: 'Unauthorized', meta: null }, 401);
