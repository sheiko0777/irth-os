...
aramexWebhookRoute.post("/", async (c: Context) => {
  ...
  if (!token) {
    return c.json(
      { data: null, error: "webhook_token_not_configured", meta: null },
      500,
    );
  }
  ...
  const hashedHeaderToken = createHash("sha256").update(headerToken).digest();
  ...
  const trackingNumber =
    payload.WaybillNumber || payload.ShipmentTrackingNumber;
