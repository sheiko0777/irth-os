...
aramexWebhookRoute.post('/', async (c: Context) => {
  ...
  // Prevent length-based timing leaks by hashing both strings to a constant length before comparing
  const hashedExpected = createHash('sha256').update(token).digest();
  const hashedProvided = createHash('sha256').update(headerToken).digest();

  if (!timingSafeEqual(hashedProvided, hashedExpected)) {
