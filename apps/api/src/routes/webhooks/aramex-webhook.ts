...
aramexWebhookRoute.post('/', async (c: Context) => {
  ...
  const hashedHeaderToken = crypto.createHash('sha256').update(headerToken).digest();
  const hashedExpectedToken = crypto.createHash('sha256').update(token).digest();

  if (!crypto.timingSafeEqual(hashedHeaderToken, hashedExpectedToken)) {
