...
import cryptoNode, { timingSafeEqual } from 'node:crypto';
...
bostaWebhookRoute.post('/', async (c: Context) => {
  ...
  const hashedSig = cryptoNode.createHash('sha256').update(sigBuf).digest();
  const hashedExp = cryptoNode.createHash('sha256').update(expBuf).digest();
