...
const bostaSchema = z.object({
    trackingNumber: z.string(),
    state: z.string()
}).passthrough();
bostaRoute.post('/', async (c) => {
    ...
    // Zod parsing
    const parsedBody = bostaSchema.parse(JSON.parse(bodyRaw));
    const { trackingNumber, state: bostaState } = parsedBody;
    // We enforce scoping for this webhook by extracting it from query param (added during shipping create)
    const orgId = c.req.query('orgId');
    if (!orgId) {
        return c.json({ data: null, error: 'missing_orgId_in_query', meta: null }, 400);
    }
    const [shipment] = await db.select().from(shipmentTracking).where(and(eq(shipmentTracking.trackingNumber, trackingNumber), eq(shipmentTracking.orgId, orgId)));
    if (!shipment) {
        return c.json({ data: null, error: 'shipment_not_found', meta: null }, 404);
    }
    await db.update(shipmentTracking)
        .set({ status: bostaState, rawPayload: parsedBody, updatedAt: new Date() })
        .where(and(eq(shipmentTracking.id, shipment.id), eq(shipmentTracking.orgId, orgId)));
    ...
    if (newOrderStatus) {
        const [order] = await db.select().from(orders).where(and(eq(orders.id, shipment.orderId), eq(orders.orgId, orgId)));
