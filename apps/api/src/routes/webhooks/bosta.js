...
bostaRoute.post('/', async (c) => {
    ...
    const bostaState = payload.state;
    if (!trackingNumber) {
        return c.json({ data: null, error: 'missing_tracking_number', meta: null }, 400);
    }
    const [shipment] = await db.select().from(shipmentTracking).where(and(eq(shipmentTracking.trackingNumber, trackingNumber), eq(shipmentTracking.orgId, c.req.header("org_id") || "system")));
    if (!shipment) {
