...
bostaRoute.post("/", async (c: Context) => {
  ...
  const calculatedBuf = encoder.encode(calculatedHmac);
  const headerBuf = encoder.encode(hmacHeader);

  if (
    calculatedBuf.length !== headerBuf.length ||
    !crypto.timingSafeEqual(calculatedBuf, headerBuf)
  ) {
