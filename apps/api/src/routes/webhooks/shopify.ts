...
import {
  ...
  type ShopifyCustomerPayload, type ShopifyOrderPayload,
...
shopifyWebhookRoute.post('/orders-create', verifyShopifyWebhook(), async (c: Context) => {
  ...
  try {
    ...
    // Never a partial import. Checked before any stock is touched: if even one
    // line has no local variant, the order is written BLOCKED — no items, no
    // stock movement, no customer notification, no ETA — and every line stays
    // on source_payload for the operator to map and re-import. Previously the
    // unmatched lines were skipped and the order saved with Shopify's full
    // total, so its items silently did not add up.
    const unmapped = await findUnmappedLines(tx, orgId, payload);
    if (unmapped.length > 0) {
      const seqBlocked = await nextDocumentNumber(tx, orgId, 'order');
      const blockedNumber = formatDocumentNumber('order', seqBlocked);
      const blockedOrder = await withAudit(tx, async () => {
        const [row] = await tx.insert(orders).values({
          orgId,
          orderNumber: blockedNumber,
          status,
          paymentMethod,
          ...
          blockedReason: blockedReasonFor(unmapped),
      ...
      await notifyAdminsOfBlockedImport(tx, orgId, payload.name, unmapped.length);
      if (delivery.kind !== 'unrecorded') await markDeliveryProcessed(tx, delivery.deliveryId);
      return { order: blockedOrder, unmatchedSkus: unmapped.map((u) => u.label) };
    }
