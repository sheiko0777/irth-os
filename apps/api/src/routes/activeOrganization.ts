...
import { orgMembers, jsonSafe } from '@irth/db';
...
activeOrganizationRoute.post('/', async (c) => {
  ...
  if (!userId) return c.json(jsonSafe({ data: null, error: 'Unauthorized', meta: null }), 401);

  let body: unknown;
  try { body = await c.req.json(); } catch { return c.json(jsonSafe({ data: null, error: 'Invalid JSON', meta: null }), 400); }
  const parsed = bodySchema.safeParse(body);
  if (!parsed.success || !isOrganizationId(parsed.data.organizationId)) {
    return c.json(jsonSafe({ data: null, error: 'Invalid organizationId', meta: null }), 400);
  }
  ...
  const [membership] = await getDb().select({ orgId: orgMembers.orgId })
    ...
    .limit(1);
  if (!membership) return c.json(jsonSafe({ data: null, error: 'Organization access denied', meta: null }), 403);
  ...
  return c.json(jsonSafe({ data: { organizationId: membership.orgId }, error: null, meta: null }));
});
...
activeOrganizationRoute.delete('/', async (c) => {
  ...
  return c.json(jsonSafe({ data: null, error: null, meta: null }));
});
