...
categoriesRouter.post('/', requireRole('owner', 'admin'), async (c) => {
    try {
        const orgId = c.req.header('org_id');
        if (!orgId)
            return c.json({ data: null, error: 'Unauthorized', meta: null }, 401);
...
categoriesRouter.delete('/:id', requireRole('owner'), async (c) => {
    try {
        const orgId = c.req.header('org_id');
        if (!orgId)
            return c.json({ data: null, error: 'Unauthorized', meta: null }, 401);
        ...
        const userId = c.req.header('user_id') || 'system';
        const result = await withAudit(db, async () => {
            const [deleted] = await db.delete(categories).where(and(eq(categories.id, id), eq(categories.orgId, orgId))).returning();
            return deleted;
        }, {
            orgId,
            userId,
