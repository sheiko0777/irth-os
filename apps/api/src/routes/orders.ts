...
ordersRoute.post('/', async (c: Context) => {
  ...
  if (!data.items.length) {
    return c.json({ data: null, error: 'empty_order', meta: null }, 400);
  }

  let totalAmount = 0;
  const itemsToInsert: { orgId: string, variantId: string, quantity: number, price: string }[] = [];
  
  // ⚡ BOLT OPTIMIZATION: Replaced O(N) queries in loop with O(1) query using inArray
  ...
  // Build a lookup map for O(1) checks
  ...
  // ⚡ BOLT OPTIMIZATION: Use sql`count(*)` instead of materializing the entire table into memory just to check its length
  const [{ count }] = await db.select({ count: sql<number>`count(*)` }).from(orders).where(eq(orders.orgId, orgId));
  ...
  // ⚡ BOLT OPTIMIZATION: Batched inserts avoiding N+1 queries in loop
