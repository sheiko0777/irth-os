...
ordersRoute.post('/', async (c: Context) => {
  ...
  const [existingOrdersCount] = await db.select({ count: count() }).from(orders).where(eq(orders.orgId, orgId));
  const seq = (existingOrdersCount.count + 1).toString().padStart(4, '0');
  ...
  if (finalItemsToInsert.length > 0) {
    await db.insert(orderItems).values(finalItemsToInsert);
  }
