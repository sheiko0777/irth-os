...
ordersRoute.post('/', async (c: Context) => {
  ...
  if (data.items.length === 0) {
    return c.json({ data: null, error: 'order_items_empty', meta: null }, 400);
  }

  let totalAmount = 0;
  const itemsToInsert: { orgId: string, variantId: string, quantity: number, price: string }[] = [];

  // ⚡ Bolt: Batch fetch variants to avoid N+1 queries
  ...
  // ⚡ Bolt: Use aggregate function instead of loading all rows into memory
  ...
  // ⚡ Bolt: Batch insert order items to avoid N+1 inserts
