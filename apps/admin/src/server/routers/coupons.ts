      ...
      return withAudit(
        ...
        async () => {
          ...
          if (input.value !== undefined)
            formattedData.value = input.value.toString();
          if (input.minOrderAmount !== undefined)
            formattedData.minOrderAmount =
              input.minOrderAmount?.toString() ?? null;
  ...
  validate: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (
        ...
        input.orderAmount < Number(coupon.minOrderAmount)
      ) {
        return {
          valid: false,
        ...
        async () => {
          ...
          if (!result.length) {
            ...
            throw new TRPCError({
              code: "BAD_REQUEST",
              message:
                "Coupon is not redeemable (inactive, expired, or usage limit reached)",
            });
          }
