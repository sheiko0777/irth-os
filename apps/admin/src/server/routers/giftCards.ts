...
export const giftCardsRouter = router({
  ...
  summary: protectedProcedure.query(async ({ ctx }) => {
    ...
    const totalIssuedMinor = rows.reduce(
      (s, r) => s + r.initialAmountMinor,
      BigInt(0),
    );
  ...
  create: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const initialAmountMinor = parseDecimal(
        String(input.initialAmount),
        giftCardCurrency,
      ).minor;
          ...
          await postJournalEntry(tx, {
            ...
            lines: [
              // Provisional: defaulted to EGP pending gift_cards acquiring a currency column.
              {
                ...
                currency: assertSupportedCurrency("EGP"),
                debitMinor: initialAmountMinor,
              },
              {
                accountCode: ACCOUNT_CODES.GIFT_CARD_LIABILITY,
                currency: assertSupportedCurrency("EGP"),
                creditMinor: initialAmountMinor,
              },
            ],
          });
        }
      ...
      ctx.idempotent(
        ...
        async () => {
          const [card] = await ctx.db
            ...
            .from(giftCards)
            .where(
              and(eq(giftCards.id, input.id), eq(giftCards.orgId, ctx.orgId)),
            );
          ...
          const amountMinor = parseDecimal(
            String(input.amount),
          ...
          const updated = await ctx.withOrg(async (tx) => {
            ...
            await withAudit(tx, async () => ({ id: input.id }), {
              orgId: ctx.orgId,
              userId: ctx.userId,
      ...
      ctx.idempotent(
        ...
        async () => {
          ...
          if (!updated) {
            throw new TRPCError({
              code: "BAD_REQUEST",
              message: "رصيد غير كافٍ أو البطاقة غير نشطة",
            });
          }
  ...
  cancel: ownerProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!updated) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "لا يمكن إلغاء بطاقة مستخدمة",
        });
      }
