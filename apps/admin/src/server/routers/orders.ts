...
export const ordersRouter = router({
    ...
    getById: protectedProcedure
        ...
        .query(async ({ ctx, input }) => {
            // Batch sequential queries concurrently to reduce max latency
            const [order, items, history] = await Promise.all([
                ...
                ctx.db
                    ...
                    .where(and(
                        eq(orderItems.orderId, input.id), // Use input.id to break dependency
                ...
                ctx.db
                    ...
                    .where(and(
                        eq(shipmentTracking.orderId, input.id), // Use input.id to break dependency
