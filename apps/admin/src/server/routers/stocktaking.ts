    ...
    create: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const session = await ctx.withOrg(async (tx) => {
          ...
          // Initialize stocktaking items from active variants and inventory
          ...
          if (variantsWithStock.length > 0) {
            const itemsToInsert = variantsWithStock.map((row) => ({
              sessionId: newSession.id,
              orgId: ctx.orgId,
              productId: row.productId,
              variantId: row.variantId,
              sku: row.sku,
              productName: `${row.productNameAr || row.productName}${
                row.variantName && row.variantName !== 'Default Title' && row.variantName !== 'الأساسي'
                  ? ` - ${row.variantName}`
                  : ''
              }`,
    ...
    recordScan: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        return await ctx.withOrg(async (tx) => {
          ...
          // 1. Search for existing item in this session
          ...
          // 2. If not in session, resolve via productVariants
          ...
          // 3. If not in variants, resolve via products directly
