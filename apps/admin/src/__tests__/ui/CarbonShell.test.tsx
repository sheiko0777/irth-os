...
vi.mock("@/components/layout/AlertPanel", () => ({
  AlertPanel: () => <div>Operational alerts</div>,
}));
vi.mock("@/components/layout/OrgSwitcher", () => ({
  OrgSwitcher: () => <div>Organization switcher</div>,
}));
...
describe("legacy shell characterization", () => {
  ...
  it("opens the existing command palette event", async () => {
    ...
    await userEvent.click(
      screen.getAllByRole("button", { name: "فتح لوحة الأوامر" })[0],
    );
...
describe("Carbon review shell", () => {
  ...
  it("preserves platform admin visibility rules", () => {
    ...
    view.rerender(
      <CarbonShell locale="ar">
        <h1>Existing page</h1>
      </CarbonShell>,
    );
    expect(
      screen.getByRole("link", { name: "لوحة الأدمن" }).getAttribute("href"),
    ).toBe("/ar/platform-admin");
  });
  it("uses the existing palette event", async () => {
    ...
    mount();
    await userEvent.click(
      screen.getByRole("button", { name: "فتح لوحة الأوامر" }),
    );
    expect(listener).toHaveBeenCalledOnce();
    window.removeEventListener("irth:palette", listener);
  });
  ...
  it("keeps English locale URLs and does not override document direction", () => {
    ...
    expect(
      screen.getByRole("link", { name: "الطلبات" }).getAttribute("href"),
    ).toBe("/en/orders");
