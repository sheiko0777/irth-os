...
function createAuth() {
  return betterAuth({
    ...
    plugins: [
       ...
       *
       * Re-added after an emergency revert: the actual break was
       * better-auth 1.7.4's drizzle-adapter schema validation rejecting
       * `twoFactor`'s userId FK for having no matching relations()
       * declaration (see packages/db/src/schema/auth.ts's
       * twoFactorRelations, added alongside this). Not a config problem
       * with this plugin call itself.
       */
      twoFactor({
